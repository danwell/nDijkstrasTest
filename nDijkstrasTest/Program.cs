...
/*
		private static int[,] GetMovementRange(Map map, Entity theEntity)
		{
			var visits = Generate2DArrayWithDefaultValue(map.Width, map.Height, -1);
			...
			// Expend movements points to go onto another tile. Points must be >= cost of terrain to traverse
			...
			while (toVisit.Count > 0)
			{
				...
				var visitTile = visits[visitLocation.X, visitLocation.Y];
				var targetTile = map[visitLocation.X, visitLocation.Y];
				// Already visited?
				if (visitTile != -1)
				{
					// Assuming not obstructed
					// Do we have a higher movement score?
					if (visitScore <= visitTile)
						continue;

					// Overwrite it and flag it for revisit
					visits[visitLocation.X, visitLocation.Y] = visitScore - targetTile.MovementCost;
					toVisit.Enqueue(new Transformable() { X = targetX, Y = targetY });
				}
				else
				{
					if (targetTile.IsObstruction) continue;
					...
					if (map[targetX, targetY].MovementCost <= visitScore)
					{
						...
						// Flag our neighbours for visiting
						// Visit above
						if ((visitLocation.Y - 1) >= 0)
						{

							visits[visitLocation.X, visitLocation.Y] = visitScore;
							toVisit.Enqueue(new Transformable()
							{
								X = visitLocation.X,
								Y = visitLocation.Y - 1
